...
extension InputEditor: UITextViewDelegate {
    ...
    public func textView(_ textView: UITextView, editMenuForTextIn _: NSRange, suggestedActions: [UIMenuElement]) -> UIMenu? {
        let actions: [UIAction] = [
            UIAction(title: String(localized: "Insert New Line", bundle: .module)) { _ in
                textView.insertText("\n")
            },
            UIAction(title: String(localized: "More", bundle: .module)) { [weak self] _ in
    ...
    func updateTextHeight() {
        ...
        withAnimation { self.textHeight.send(decision) }
    }
