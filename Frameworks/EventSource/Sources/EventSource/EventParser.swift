...
private extension Data {
    ...
    func split(by separator: [UInt8]) -> [Data] {
        ...
        while true {
            range = self.range(of: doubleNewline, options: [], in: currentIndex ..< count)
            if let foundRange = range {
                splits.append(subdata(in: currentIndex ..< foundRange.lowerBound))
                currentIndex = foundRange.upperBound
            } else {
                splits.append(subdata(in: currentIndex ..< count))
                break
            }
        }
