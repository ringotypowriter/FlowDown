...
extension UIConversation.MessageListView.TableLayoutEngine {
    protocol LayoutableCell: AnyObject {
        static func resolveLayout(
            dataElement: UIConversation.MessageListView.Element,
...
extension UIConversation.MessageListView.TableLayoutEngine {
    @discardableResult
    func resolveLayoutNow(_ element: UIConversation.MessageListView.Element) -> LayoutCache {
        var hasher = Hasher()
    ...
    func requestLayoutCacheFromCell(
        forElement dataElement: UIConversation.MessageListView.Element,
...
extension UIConversation.MessageListView.TableLayoutEngine {
    func height(forElement dataElement: UIConversation.MessageListView.Element) -> CGFloat? {
        accessLayoutCache { pool in
            guard let box = pool[dataElement.id] else { return nil }
            guard box.contentIdentifier == dataElement.object?.hashValue else { return nil }
            guard box.cache.width == contentWidth else { return nil }
