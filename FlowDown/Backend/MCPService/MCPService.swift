...
class MCPService : NSObject {
    ...
    private func setSubscriptions() {
        ...
        clientConfigs.map{ clients in
            clients.filter {$0.isEnabled}
        }
        .sink{[weak self] enabledClients in
            self?.enabledMCPClientsCfg.send(enabledClients)
            print("[+] Enabled MCP Clients: \(enabledClients.map { $0.name })")
        }
    ...
    private func loadFromDatabase() {
        self.clientConfigs.send(self.scanMCPClients())
    }
    // MARK: - Active Clients Management
    private func createActiveClient(enabledClients: [MCPClient]) {
