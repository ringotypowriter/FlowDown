...
extension ModelManager.PromptType {
    func createPrompt() -> String {
        ...
        case .minimal:
            ...
            When presenting content in a list, task list, or numbered list, avoid nesting code blocks or tables. Code blocks and tables in Markdown syntax should only appear at the root level. For complex answers, organise with headings ## / ### and separate sections using ---; use lists, **bold**, and _italics_ as needed. Never use tildes unless you genuinely need ~strikethrough~. Use LaTeX for all math: \\( ... \\) for inline math like \\( E=mc^2 \\), and \\[ ... \\] for display math. For multi-line equations, use an `align` environment within the display delimiters, like so: \\[ \begin{align*} a &= b \\ c &= d \end{align*} \\]. You understand that markdown may escape your symbols, in that case, add \ prior to the symbol to escape it. Eg: \( will be output as \\\(, ~ will need \~. Otherwise it will be parsed accordingly.
        ...
        case .complete:
            ...
            For all mathematical content, ranging from single variables to complex multi-line formulas, you must consistently use LaTeX formatting to ensure clarity and proper rendering. For inline mathematics that flows within a sentence, enclose the expression in \\( ... \\), for example, \\( E=mc^2 \\). For standalone, display-style equations that should be centered on their own line, use \\[ ... \\], such as \\[ \int_{a}^{b} f(x) \,dx = F(b) - F(a) \\]. When an equation requires multiple aligned lines, use the `align` environment within the display delimiters to ensure proper structure and readability, for instance, \\[ \begin{align*} f'(x) &= (x^2)' \\ &= 2x \end{align*} \\].
