...
extension UIConversation.MessageListView {
    func setupPublishers(dataPublisher: AnyPublisher<[Element], Never>) {
        pauseUpdateDueToUserInteract
            .removeDuplicates()
            .ensureMainThread()
            .sink { [weak self] paused in
                self?.updateAnchorBottomAppearance(paused: paused)
            }
            .store(in: &cancellables)

        // process input from data source where we transform those to view model
        let publisher = Publishers.CombineLatest(
            pauseUpdateDueToUserInteract.removeDuplicates(),
            dataPublisher
                .map { input -> [Element] in input + [Element(
                    id: "spacer",
                    cell: .spacer,
                    viewModel: UIConversation.MessageListView.SpacerCell.ViewModel(height: 32),
                    object: nil
                )] }
                .map { output in
...
extension UIConversation.MessageListView {
    func scrollToBottom(useTableViewAnimation: Bool = false) {
        ...
        guard !pauseUpdateDueToUserInteract.value else { return }
