...
struct FlowLayout: Layout {
    ...
    func sizeThatFits(proposal: ProposedViewSize, subviews: Subviews, cache _: inout Void) -> CGSize {
        let rows = computeLayout(for: proposal, subviews: subviews)
        return sizeThatFits(rows: rows)
    }

    func placeSubviews(
        in bounds: CGRect, proposal: ProposedViewSize, subviews: Subviews, cache _: inout Void
    ) {
        let rows = computeLayout(for: proposal, subviews: subviews)
        ...
        for row in rows {
            for item in row.items {
                ...
                position.x += item.size.width + horizontalSpacing
            }

            position.x = bounds.origin.x
            position.y += row.size.height + verticalSpacing
        }
    }
}
    ...
    private func computeLayout(for proposal: ProposedViewSize, subviews: Subviews) -> [Row] {
        ...
        for (index, view) in zip(subviews.indices, subviews) {
               ...
               currentRow.size.width + item.size.width > (proposal.width ?? .infinity)
            {
                ...
                currentRow.size.width -= horizontalSpacing
                rows.append(currentRow)
                currentRow = Row()
            }

            currentRow.items.append(item)
            currentRow.size.width += item.size.width + horizontalSpacing
