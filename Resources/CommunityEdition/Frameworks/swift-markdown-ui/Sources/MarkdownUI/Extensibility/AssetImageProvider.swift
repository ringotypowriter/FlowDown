...
public struct AssetImageProvider: ImageProvider {
    ...
    public func makeImage(url: URL?) -> some View {
        if let url, let image = image(url: url) {
    ...
    private func image(url: URL) -> PlatformImage? {
        #if canImport(UIKit)
            return UIImage(named: name(url), in: bundle, with: nil)
        #elseif canImport(AppKit)
            if let bundle, bundle != .main {
                return bundle.image(forResource: name(url))
            } else {
                return NSImage(named: name(url))
            }
        #endif
    }
}
